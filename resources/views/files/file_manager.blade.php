...
<form action="{{ route('files.bulk-action') }}" method="POST" id="bulkActionForm">
    ...
    <div class="form_actions_count">
        @if(request()->has('search') || request()->has('uploader'))
    ...
    <thead>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><a href="{{ route('files.download', $file->id) }}">{{ $file->filename }}</a></td>
    ...
    @if ($file->expires && $file->expiry_date)
        ...
        @else
            <span class="label label-success" style="display: block; text-align: center;">
                Válido hasta<br>
                <strong>{{ $expiryDate->format('Y/m/d') }}</strong>
            </span>
        @endif
    @else
        <span class="label label-no-expire" style="background-color: #dff0d8; color: #3c763d; padding: 5px 10px; border-radius: 3px; font-weight: bold;">
            No expira
        </span>
    @endif
</td>
    ...
    // Validar que se seleccione al menos un archivo y una acción válida
    ...
    if (action === 'delete') {
        ...
        // Enviar el formulario después de un pequeño retraso para mostrar el modal
        setTimeout(function () {
            document.getElementById('bulkActionForm').submit();
        }, 1000);
    }
