...
<body class="manage-files logged-in logged-as-admin menu_hidden backend">
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-12">
                        ...
                        <form action="{{ route('files.group-bulk-action', $group->id) }}" method="POST" id="bulkActionForm">
    @csrf
    ...
    <table id="files_tbl" class="footable table">
            ...
            <tr>
                ...
                <th>
                    <a href="{{ request()->fullUrlWithQuery(['sort' => 'filename', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}">
                        Título
                        <i class="fa fa-sort{{ request('sort') === 'filename' ? (request('direction') === 'asc' ? '-asc' : '-desc') : '' }}"></i>
                    </a>
                </th>
                <th>Tamaño</th>
                <th>
                    <a href="{{ request()->fullUrlWithQuery(['sort' => 'uploader', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}">
                        Cargador
                        <i class="fa fa-sort{{ request('sort') === 'uploader' ? (request('direction') === 'asc' ? '-asc' : '-desc') : '' }}"></i>
                    </a>
                </th>
                <th>
                    <a href="{{ request()->fullUrlWithQuery(['sort' => 'public_allow', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}">
                        Permisos públicos
                        <i class="fa fa-sort{{ request('sort') === 'public_allow' ? (request('direction') === 'asc' ? '-asc' : '-desc') : '' }}"></i>
                    </a>
                </th>
                <th>Expira</th>
                <th>Estado</th>
                <th>
                    <a href="{{ request()->fullUrlWithQuery(['sort' => 'download_count', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}">
                        ...
                        <i class="fa fa-sort{{ request('sort') === 'download_count' ? (request('direction') === 'asc' ? '-asc' : '-desc') : '' }}"></i>
                    </a>
                </th>
        ...
        <tbody>
            ...
            <tr>
                <td><input type="checkbox" name="batch[]" value="{{ $fileRelation->file_id }}"></td>
                ...
                <td>
                    @if ($fileRelation->file)
                    <a href="{{ route('companies.files.edit', $fileRelation->file_id) }}" class="btn btn-sm btn-primary">
